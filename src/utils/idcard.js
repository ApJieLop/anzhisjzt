 ...
 *
 * 使用
 *     let x = IdentityCodeValid(身份证号码);
 * 返回参数
 *     x.tip   验证描述
 *     x.pass  true/false
 **/

export function IdentityCodeValid(val) {
  let tip = "";
  let pass = true;
  ...
  //校验长度，类型
  else if (isCardNo(val) === false) {
    tip = "您输入的身份证号码不正确";
    pass = false;
  }
  //检查省份
  else if (checkProvince(val) === false) {
    tip = "您输入的身份证号码不正确";
    pass = false;
  }
  //校验生日
  else if (checkBirthday(val) === false) {
    tip = "您输入的身份证号码生日不正确";
    pass = false;
  }
  //检验位的检测
  else if (checkParity(val) === false) {
    tip = "您的身份证校验位不正确";
    pass = false;
  } else {
    tip = "";
